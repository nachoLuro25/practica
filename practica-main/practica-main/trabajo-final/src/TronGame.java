...
public class TronGame extends JPanel implements ActionListener, KeyListener {
    ...
    private int imageOffsetX = -17;  // Ajuste horizontal (negativo = izquierda, positivo = derecha)
    private int imageOffsetY = -17;  // Ajuste vertical (negativo = arriba, positivo = abajo)
    ...
    private void updateImages() {
        try {
            ...
            // Fallback: usar imágenes por defecto si hay error
            try {
    ...
    // Método para obtener el ángulo de rotación según la dirección
    private double getRotationAngle(char direction) {
        switch (direction) {
            case 'U': return -Math.PI / 2;  // -90 grados (arriba)
            case 'D': return Math.PI / 2;   // 90 grados (abajo)
            case 'L': return Math.PI;       // 180 grados (izquierda)
            case 'R': return 0;             // 0 grados (derecha)
            default: return 0;
        }
    }

    // Método para dibujar imagen rotada
    private void drawRotatedImage(Graphics2D g2d, Image image, int x, int y, int width, int height, double angle, float alpha) {
        ...
        // Configurar transparencia si es necesaria
        ...
        // Restaurar el transform y composite originales
    ...
    public void draw(Graphics g) {
        ...
        // Activar antialiasing para mejor calidad de rotación
        ...
        if (running) {
            ...
            // Dibuja las motos con rotación según su dirección
            ...
            if (imgPlayer1 != null) {
                ...
                // Fallback: dibujar un rectángulo del color del jugador
                g2d.setColor(player1Color);
                if (player1Invisible) {
